...
def _situation_for(framing_hint: str, part: str, *, has_companion: bool) -> str:
...
def _frame(now: datetime, *, lead: str, companion: str, framing_hint: str) -> ShowFrame:
    ...
    return ShowFrame(
        ...
        situation=_situation_for(framing_hint, part, has_companion=bool(companion)),
    )
...
def show_frame(now: datetime, *, night_host: str, day_host: str) -> ShowFrame:
    ...
    return _frame(now, lead=lead, companion=companion, framing_hint=hint)


def program_frame(now: datetime, program: Program) -> ShowFrame:
    ...
    if program.framing == "legacy":
        ...
        return show_frame(now, night_host=night, day_host=day)
    lead = hosts[0] if hosts else ""
    companion = hosts[1] if len(hosts) > 1 else ""
    return _frame(now, lead=lead, companion=companion, framing_hint=program.framing)
