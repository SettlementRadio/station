# CLAUDE.md — Settlement Radio

> This file is the project constitution. Read it before doing anything. Also read
> `docs/ARCHITECTURE.md`, `docs/CANON.md`, `docs/ROADMAP.md`, and the **active phase's** task pack
> (see "Active phase" below — for Phase D that's `docs/PHASE_D_OVERVIEW.md` + the per-sub-pack files).

## What this project is

An AI-powered fictional sci-fi **radio station** named **Settlement Radio**, broadcasting from
**600 years ahead of the real present** (the in-world year is always `real year + 600` — never
hardcode a year), as
a tribute to the science-fiction authors who shaped the genre. **Every word, reasoning step, and
world decision is produced by Claude. The entire system is built by Claude Code.** Voice
synthesis (TTS) is the one external component, because Anthropic does not make a voice product.

## Active phase

`docs/ROADMAP.md` owns the sequence. **Done:** Phase A (proof of loop), A2 (coming-soon site), B
(the mind — Postgres world-store, world clock, two-DJ room, formats, `make buffer`), and Phase C's
code track **C0–C4** (safety/continuity gates, time-aware framing, the rolling scheduler + retention
GC, disclosure-in-air, never-dead fallback) — see `docs/PHASE_C_ORIENTATION.md`.

Also done: **Phase D** (D1–D13 — the living world: bible folder, RAG, world tick, news desk,
freshness, programming grid, production, commercials, voice/roster, figures, operator manual,
talk continuity, self-memory) and **Phase R** (R1–R7 — editorial briefs + the register overhaul, the
GRID_V2 24-hour week, jingles batch 3, the living day, the panel extensions, the moving chart, and
the public web front). See `docs/PHASE_D_OVERVIEW.md` and `docs/PHASE_R_TASKS.md`.

**Two tracks now run in parallel — ask the human which to work on, don't assume:**
- **Phase Q — "Quality"** — the next *code* work, and **the gate on the C9 soak**. Master pack:
  **`docs/PHASE_Q_TASKS.md`** — the measured baseline from the 2026-07-26 external audit (§1), the
  fixes (Q0–Q7), and the loop that proves them. **Q0 is mandatory first**: it builds `make audit`
  and commits `docs/audit/2026-07-26-baseline.json`, without which no later pack has a gate. Work
  one pack per session, task by task; end each pack by running **`make gate PACK=Qn`** (it exits
  non-zero on a miss — the exit code is the answer, not a summary of it). **Q8's final audit runs in
  a separate, unbriefed session** against `docs/PHASE_Q_AUDIT_BRIEF.md` — do not summarise the fixes
  anywhere that session will read first.
- **Phase C server track (C5–C9)** — provision the VPS, Postgres-on-the-box, YouTube relay, the web
  player, the 7-day soak. Mostly ops/deploy. Pack: `docs/PHASE_C_TASKS.md`. **Soak after Q lands** —
  Q1–Q7 change what the soak would be listening to.

**Phase Q standing rules (they bind every task in that pack):**
- **The building agent never judges whether its own work succeeded** (`PHASE_Q_TASKS.md` §2d). Every
  gate resolves one of three ways — numerically by the harness, by the *operator* on a blind sample,
  or deferred to Q8. "A human reading…" always means the operator: print the sample, stop, and ask.
- **The default for any judgment call is "no change"**, and **a null result is a completed task**
  ("measured, no improvement, reverted" is success, not a reason to re-roll).
- **Never regress R1.** The plain-speech register and the `plain_register` acceptance property are
  load-bearing; the §2b guard floors are hard.

**Phase D/R load-bearing decisions (the operating model — full detail in the overview; honour these):**
- **Static bible vs dynamic world.** Canon is a hand-authored markdown **bible folder** (`docs/canon/`);
  the world is **dynamic DB state generated by an autonomous nightly tick** (gated, not per-story
  approved — the human controls the bible, not each story). **World history persists forever** (never
  GC'd like audio).
- **Seed safety.** `make seed-canon` (safe, everyday) vs `make reset-world` (destructive, warned);
  `reset-world` wipes world+canon only, never config/catalog. **Schema changes land via migrations,
  never truncate-reseed, once the world is alive.** (See the §2a matrix for per-table ownership/backup.)
- **Reuse the seams.** A new format is a `formats` registry entry; one **polymorphic embeddings table**;
  a **batch path behind `providers/llm`** (Batch must not leak into the tick); voice/figures stay behind
  the TTS/store seams.
- **Public read-only vs private operator.** The web/now-playing surface is public + read-only; the
  status console / admin / control surface is **private (VPS-only, single-operator)** — never exposed.

Build only what the active sub-pack calls for; everything else is deferred to its pack.

**Do not document a command before it exists.** `make audit` / `make gate` are built by Q0 — until
then they belong in the Phase Q pack, not in `README.md` or `ADMIN_MANUAL.md`. Doc drift was itself
an audit finding; don't add more of it.

## Tech reality (important)

- The human works **only through Claude Code**. You (the agent) do the building. Prefer small,
  verifiable steps and explain how to check each one.
- **Language:** Python 3.11+ for the pipeline; Liquidsoap's own config language for playout.
- **Platform:** macOS (Apple Silicon). Install system tools with Homebrew. You may run install
  commands, but show them and put them in the README.
- **Intelligence:** Anthropic API (Claude), with **model routing by job** (map these logical
  tiers in `src/providers/llm.py`):
  - `sonnet` → **Claude Sonnet 4.6** (`claude-sonnet-4-6`) — the DEFAULT writing brain: DJ
    scripts, showrunner, continuity. Script quality is the product, so this is the workhorse.
  - `haiku` → **Claude Haiku 4.5** (`claude-haiku-4-5-20251001`) — high-volume, low-stakes, and
    near-live: time checks, idents, filler, the future 60-second drops.
  - `opus` → **Claude Opus 4.8** (`claude-opus-4-8`) — hard reasoning only (gnarly continuity,
    big worldbuilding calls); runs rarely.
  - Do NOT use Fable 5 / Mythos as a workhorse — overkill and ~2x Opus cost.
- **Cost levers are mandatory, not optional** (text cost must stay near-trivial):
  - **Batch API** — the nightly generation is a batch job; run it through the Batch API (50% off).
  - **Prompt caching** — cache the stable canon / character cards / system prompt so each call
    pays full price only for the small variable part (~90% off cached input). The provider
    abstraction must support a cached-context path from the start.
- **Voice:** **Kokoro** (self-hosted, local, free, unlimited) is the **default** backend as of Phase B;
  **ElevenLabs** (cloud/flagship) stays switchable via `settings.tts_provider`. Both go ONLY through the
  TTS abstraction. **Emotion needs the flagship path — Kokoro can't carry it (D9).** The launch-voice
  choice (Kokoro-at-scale vs paid flagship) is the open **C6** decision.
- **Playout:** Liquidsoap + Icecast + ffmpeg (Homebrew).
- **Data:** **Postgres world-store from Phase B** (seeded from the canon; `settings.database_url`; all
  SQL behind `src/world/store.py`). Phase A had no DB. The canon is `docs/CANON.md` today and becomes a
  **`docs/canon/` bible folder** in Phase D (D1); pgvector/semantic recall arrives in D2.

## The two load-bearing seams (build these now, thin)

1. **Provider abstraction.** All model/voice calls go through `llm.generate(...)` and
   `tts.synthesize(text, voice, emotion)`. **Never call a vendor SDK directly outside these two
   modules.** Implementations are selected by env/config.
2. **Segment abstraction.** A `Segment` is the unit of everything: audio + metadata (`id`,
   `length_target`, `air_time`, `format`, `disclosure`). **Segment length and lead-time are
   parameters, never hardcoded** — this is what later lets a 3-hour overnight block and a
   60-second near-live drop share one code path.

See `docs/ARCHITECTURE.md` for the concrete interfaces and the `Segment` shape.

## Repo conventions

- Structure: `src/` (code), `config/` (liquidsoap + icecast configs), `docs/`, `segments/`
  (generated audio — gitignored), `assets/` (jingles/beds — gitignored), `.env` (secrets —
  gitignored), `.env.example` (empty keys, committed).
- **Secrets only in `.env`, NEVER committed.** Always keep `.env.example` current.
- Small modules, clear names, type hints. Add a test only where the logic is non-trivial.
- Every dependency and command goes in the `README.md` so the build is reproducible from scratch.

### Monorepo: the web app (`/web`)

The repo is a monorepo. The Python **station backend** stays at the root (`src/`, `config/`, etc.)
and is never built or deployed by Vercel. Alongside it, **`/web`** is a self-contained Next.js
(App Router + TypeScript + Tailwind) app — the public site, deployed to **settlementradio.com** via
**Vercel with Root Directory = `web`**. It started as the A2 coming-soon page and **grows into the
public audio player in Phase C (C8)** (new routes in the same app, pointing at the live stream +
now-playing). This app is **public and read-only** — the operator/admin/control surface is NOT here
(it's private, VPS-only — see the hard rule). The two halves run independently, so web work and the
Python pipeline don't block each other. Web secrets live in `web/.env.local` (and as Vercel env
vars), separate from the root `.env`.

## Engineering standards (the station backend — apply from now on)

These keep an unattended, 24/7, long-lived system maintainable. They apply to the Python
**station backend**. The `/web` coming-soon app may be pragmatic/temporary and need not meet all
of these.

- **Config over hardcoding.** All tunable values — model tiers, file paths, DB connection,
  segment defaults, buffer depth, provider names — come from ONE typed settings module
  (`src/config.py`, loaded from env via `pydantic-settings`). No magic numbers or literal config
  strings scattered in code. Code reads `settings.X`, never a raw literal.
- **Structured logging, never `print()`.** Use Python `logging` (structured/JSON, e.g.
  `structlog`) configured once, with levels (debug/info/warning/error). Every external call and
  every batch step logs start/outcome. This is how you'll diagnose a 3 a.m. failure you didn't
  watch — it is not optional polish for a 24/7 system.
- **Error handling + retries on external calls.** Wrap Claude / TTS / DB calls with sensible
  failure handling and bounded retries; on failure, fall back or fail loudly into the logs — never
  silently produce nothing.
- **Lint + format automatically.** Use `ruff` (lint + format) configured in `pyproject.toml`, and
  run it (plus a fast secrets scanner) on every commit via **pre-commit** hooks. Keep hooks fast —
  no test suite in pre-commit, or it gets bypassed.
- **Type hints throughout**, especially on the seams and public functions.
- **Tests are surgical, not exhaustive.** Test the bits with real logic where a silent bug would
  hurt (e.g., the world clock and the relative-time renderer). Don't chase coverage on glue code.
- **Secrets:** only in `.env` locally; on any server, not world-readable and never in the repo.

## Hard rules — never violate

- **NEVER commit secrets or API keys.**
- **IP boundary — this is a TRIBUTE, not a derivative.** Do not use real franchises, named
  characters, trademarks, or any living author's creations. Original world only — the *spirit* of
  the genre, never its intellectual property.
- **Content safety:** before any *public* broadcast (Phase C onward), generated text must pass a
  safety gate. Not required in Phase A (nothing is public), but structure the script step so a
  gate can slot in cleanly later.
- **AI disclosure:** when public, the station must disclose that it is AI-generated (spoken + on
  the player). Live since C3 (spoken ident + player line); keep the public surfaces disclosing.
- **Public read-only, admin private.** The public surface (web player, now-playing feed) is **read-only
  and carries only public-safe fields**. The operator/admin/control surface (status console; the Phase E
  panel) is **private — VPS-only, single-operator, never internet-exposed.** Open-source code is fine;
  security comes from secrets + access control, never code secrecy.
- **Ask before destructive actions** (deleting files, force operations, anything irreversible). Note
  `make reset-world` is destructive (wipes the living world) — it must warn + confirm; `make seed-canon`
  is the safe everyday seed.

## How to work (agent instructions)

- Read `docs/ARCHITECTURE.md` and `docs/CANON.md`, plus the task pack for what you're building, before
  coding (`docs/ROADMAP.md` lists the phases; **ask the human which track/sub-pack if unsure**). For
  **Phase D**, start at `docs/PHASE_D_OVERVIEW.md` (§2 standing principles, §2a the seed/state matrix,
  the tracker), then read the specific `docs/PHASE_D_<NAME>_TASKS.md` and the files in its "Read first".
- Work **task by task** from the sub-pack, in order. After each task, state what you did and exactly how
  the human can verify it. As you finish a sub-pack, flip its row in the overview tracker (`Built? ✅`)
  and append its admin how-tos to `docs/ADMIN_MANUAL.md` (the D11 capstone consolidates them).
- Keep changes small and reviewable. Update `README.md` as you go.
- If a detail isn't specified, choose the simplest option that respects the two seams, and note
  the choice in your summary.
