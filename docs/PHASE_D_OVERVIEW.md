...
- **Depends on:** C2 scheduler, D9.4 (the pattern), D12 (the capture site + `flow`); D2
  recommended for semantic recall. **BUILT (D13.0–D13.4)** ahead of the C9 soak — revisit the
  extraction prompt against the soak's week of real output (the original sequencing advice
  stands as a tune-up, not a blocker).
...
| D13 Self & Interpersonal Memory (addendum) | `PHASE_D_SELF_MEMORY_TASKS.md` | C2 scheduler, D9.4, D12 (D2 rec.) | ✅ | ✅ | **Built** (D13.0–D13.4; `host_journal` accrual + post-air `haiku` capture at the chokepoint + D9.4-sibling recall (per-host + pair line, semantic blend) + the self-consistency gate; `make journal-demo`; seventh `journal_memory` acceptance property; card wins over journal, always) |
