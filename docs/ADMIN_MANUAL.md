...
| `make chart` (R6.1) | re-ranks the daily music chart The Count counts down (`state` row `chart:daily`; runtime — survives seed-canon, cleared by reset-world). Normally runs automatically at the tail of `make world-tick`; standalone for a quick regen/look. Cheap + deterministic-per-day; tag a track `chart` in `config/tracks.yaml` to make it chart-eligible, `featured`/`pinned` to weight it. **On the panel:** the current chart shows on **Panel → World**, with an "Update chart" run button (E1.1) — ✅ built | rarely by hand; it rides the nightly tick |
