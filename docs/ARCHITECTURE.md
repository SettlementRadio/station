...
4. **Production** — script → TTS → mix with jingles/beds → emit a `Segment`. *Phase A: TTS only;
   ...
   **ducking** (a bed sits under speech; a sting fires before news). Voice gains **emotion** (the
   ...
   reserved in Seam #1b). **Songs** enter as a *non-generated audio asset*: the Layer 5 scheduler
   drops a track from a pool into the `music` format's `[SONG]` slot — the catalog/clearance is a
   separate human call, the plumbing lives here + Layer 5.
5. **Scheduling & playout** — a scheduler decides what/when (buffer depth = a parameter);
   ...
   **Phase C (forward):** a real scheduler — rolling buffer to a depth dial, **measured** durations
   (ffprobe), regenerate-on-failure, a never-dead fallback chain. **Phase D (forward):** it reads a
   **programming model** — named programs, dayparts, a weekly routine (which show, which DJs, when) —
   and inserts **songs** into the music slot. **Phase E (forward):** the depth dial → ~0 for near-live.
...
Phase D adds non-spoken `format`s the producer doesn't *write* but the scheduler still airs —
`ident`/`sting`/`jingle` (pre-rendered assets) and `song` (a track from a pool, dropped into the
`music` slot). These carry `audio_path` with `script=None`; the Segment shape already holds them, so
...
- **Phase D — the living world:** the bible becomes a `docs/canon/` folder + pgvector (Layer 2); a
  ...
  **songs** land in Layer 4/5; voice gains **emotion + pronunciation** (Seam #1b); the scheduler
  ...
  The two seams and the Segment are unchanged — new `format`s (ident/sting/song) reuse the Segment.
