# 80-cosmos.md — the universe & the wonder

> Cornerstone file: what the worlds know of the cosmos and the awe it inspires (bible prose) +
> the standing facts. See `docs/canon/README.md` for the authoring conventions.
...
Every settled world has its own sky, but the same sky hangs above them all in a sense — the same scattered stars, the same dark between. A child on Meridian learns the constellations as they appear from that world's position, but knows that the same stars shine on Cold Harbor, on Forge, on the stations drifting between. The station's observatory dome tracks these stars, marks their slow drift, and keeps count of how few of them the settlements have ever touched.

The sky a settled child grows up under is not Earth's sky. The old constellations are gone, redrawn from a new vantage; the familiar sun is somewhere among the scattered points, no brighter than the rest. Children grow up knowing that the brightest light in their sky might be a settlement, might be a relay station, might be a ship in transit. They learn to read the difference: the steady light of a world, the flicker of a station's rotation, the slow traverse of a vessel between relays.
