...
| `domains` | **(R4.3, optional)** the **world-domains this vertical covers** — a list of tick domain tags (`finance`, `health`, `sports`, `nations`, `technology`, `culture`, `history`, `war`, `literature`, `geography`, `peoples`, `religion`, `style`). When set, the writers' room **prefers the story-log beats whose story is in one of these domains** (`context.assemble` surfaces them as "this show's subject", the rest as background), so The Exchange talks *this week's* trade story, not trade in the abstract. Absent = a **general** show that keeps the full mix (the flagships, the debate desk, music). Use the domains that actually generate stories — tagging a show with a domain the tick never writes (e.g. food) would starve it, so those stay general. |
